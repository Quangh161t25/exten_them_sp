...
async function getAccessToken() {
    ...
    if (typeof KJUR === "undefined" || !KJUR || !KJUR.jws || !KJUR.jws.JWS) {
        throw new Error("Thư viện jsrsasign (KJUR) chưa được nạp!");
    }
...
function renderHeaders() {
    ...
    if (currentTab === 'THU_CHI') headers.push('tiền còn lại', 'Xóa');
...
function sortThuChiRowsByNgayDesc(rows) {
    return sortRowsByNgayDesc(rows, THU_CHI_NGAY_COL);
}
...
function renderTable() {
    ...
    const thuChiBalanceByRow = currentTab === 'THU_CHI' ? buildThuChiBalanceMap(filteredData) : null;
    ...
    tbody.innerHTML = pageData.map(row => {
        ...
        if (currentTab === 'THU_CHI') {
            extraCols = `<td class="price-cell">${formatCurrency(thuChiBalanceByRow.get(row) || 0)}</td>`;
            extraCols += `<td><button type="button" class="btn-delete" onclick="deleteDataSheetRow(${sr})">Xóa</button></td>`;
        }
